...
namespace Assignment1
{
    class MainClass
    {
        public static void Main(string[] args)
        {
                ...
                else if (args[i] == "-i" || args[i] == "--input")
                {
                    ...
                    if (args.Length > i + 1)
                    {
                        ...
                        if (string.IsNullOrEmpty(inputFile))
                        {
                            ...
                            Console.WriteLine("No input file specifies");
                        }
            ...
            //Ensure file is loaded before using results.
            if (!string.IsNullOrEmpty(inputFile) && results.Count == 0)
            {
                results = Parse(inputFile);
            }
            ...
            if (sortEnabled)
            {
                ...
                {
                    Console.WriteLine("No column name specified for sorting. Defaulting to Name.");
            ...
            if (results.Count > 0)
            {
                if (!string.IsNullOrEmpty(outputFile))
                {
                    ...
                    using (StreamWriter writer = new StreamWriter(fs))
                    {
                        ...
                        Console.WriteLine($"File has been saved to {outputFile}");
                    }
                }
                else
                {
        ...
        public static List<Weapon> Parse(string fileName)
        {
            ...
            using (StreamReader reader = new StreamReader(fileName))
            {
                ...
                while (!reader.EndOfStream)
                {
                    ...
                    if (values.Length != 4)
                    {
                        Console.WriteLine("Invalid data format: {0}", line);
                        continue;
                    }
                    ...
                    if (!int.TryParse(values[2], out rarity))
                    {
                        Console.WriteLine("Invalid Rarity value: {0}", values[2]);
                        continue;
                    }

                    if (!int.TryParse(values[3], out baseAttack))
                    {
                        Console.WriteLine("Invalid BaseAttack value: {0}", values[3]);
                        continue;
                    }
